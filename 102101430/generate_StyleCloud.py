...
def read_Danmu(workbook_Name, sheet_Name):  # 从Excel表中读取数据
    ...
    #若报错，则返回空迭代器
        ...
        print("输入文件的路径或格式错误，请打开config.ini文件重新配置路径\n")
        return iter(())
    except KeyError:
        print("工作表页名错误，请检查Sheet的名字和config.ini中是否一致\n")
        return iter(())
    except:
...
def generate_Word_Cloud(counter):  # 生成词云图
    try:
        if not counter:  # 如果计数器对象为空，则给出提示并退出函数
            return "输入的词频为空！"
        stylecloud.gen_stylecloud(text=counter, icon_name='fas fa-bomb', 
                                  palette='colorbrewer.sequential.PuBu_9', size=WC_Height, 
                                  font_path=font_Path, output_name=output_Path,
...
if __name__ == "__main__":
    ...
    workbook_Name = config.get(config_Section_Name,'workbook_name',fallback='output/Top_20_danmu.xlsx')  # 要读取的Excel表的名称，默认为crawler.py生成的文件
    sheet_Name = config.get(config_Section_Name,'sheet_Name',fallback='所有弹幕')  # 要读取的Excel表的页的名称，可从['Top 20', '所有弹幕']中选择
    WC_Width = config.getint(config_Section_Name,'WC_Width',fallback=1200)  # 词云图的宽度
    WC_Height = config.getint(config_Section_Name,'WC_Height',fallback=1200) # 词云图的高度
    font_Path = config.get(config_Section_Name,'font_Path',fallback="config/msyh.ttc")  # 字体存储路径
    pic_Path = config.get(config_Section_Name,'pic_Path',fallback="config/m.png")  # 词云背景图路径
    output_Path = config.get(config_Section_Name,'output_Path',fallback="output/word_could.png")
    main()
