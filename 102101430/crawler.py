...
def get_Response(html_Url, h=header):  # 对于任意一个网址url，获取其html文件
    try:
        ...
        print(f"请求网页失败，无法连接到 {html_Url}")
        return None
    except requests.exceptions.Timeout:
        print(f"请求网页超时，网页地址：{html_Url}")
        return None
...
# 从B站搜索栏中找排名前total_Num的视频，爬取他们的BV号，target为搜索内容,step为一页读取的视频数，默认为30
def get_Target_Video(arg):
    ...
    if response.ok:
        ...
        print(f"请求网页失败,状态码：{response.status_code},可能是Cookie有误,请仔细核对")
        return list()
...
def get_Cid(BV):  # 对于一个B站视频网址，获取其cid(即存储弹幕的网站key),输入为BV号
    try:
        ...
        print(f"BV号为{BV}的视频不存在！\n")
        return None     #如果get_Response()返回None了就报错并跳过
...
def get_Danmaku(BV):  # 对于一个BV号，将其分析为cid，后进入弹幕网站并爬取弹幕进行分析
    try:
        ...
        return None     #如果get_Cid()返回None了就跳过，报错已经在get_Cid中进行了
...
def main():
    ...
    # Step 2：利用线程池对每一个视频的弹幕进行分析并分析频率
    print("Step 2: 利用线程池对每一个视频的弹幕进行爬取并分析频率")
    pool = Pool(max_Processing_Num)  # 线程太多会403、412，少用一些线程
